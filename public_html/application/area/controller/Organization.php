...
class Organization extends Base{
    ...
    public function organization_list(){//机构列表
        ...
        $seek = input('seek');
        $district_id = Session::get('district_id','area');
        $organization_select = Db::name('organization')->alias('a')->join('district b','a.organization_district_id = b.district_id','left')->join('user c','a.user_id = c.user_id','left')->where('organization_false_delete',1)->field('organization_id,organization_state,organization_name,organization_portrait,organization_attention,district_name')->where('district_id',$district_id)->where('organization_name','like','%'.$seek.'%')->paginate(15);
    ...
    public function organization_check_list(){//审核列表
        ...
        $organization_select = Db::name('organization')->alias('a')->join('district b','a.organization_district_id = b.district_id','left')->join('user c','a.user_id = c.user_id','left')->where('organization_false_delete',1)->where('organization_state',3)->where('organization_name','like','%'.$seek.'%')->where('district_id',$district_id)->paginate(15);
        $this->assign('seek',$seek);
        $this->assign('organization_select',$organization_select);
        return view();
    }
